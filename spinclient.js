  ...
  angular.module('ngSpinclient', ['uuid4', 'ngMaterial']).factory('spinclient', function(uuid4, $q, $rootScope) {
    ...
    'spinclient', '$mdDialog', function(client, $mdDialog) {
      return {
        restrict: 'AE',
        replace: true,
        template: '<div layout="row"> <div flex ng-repeat="prop in listprops" class="gray"> <input flex ng-if="prop.type && prop.value && !prop.hashtable && !prop.array" ng-click="enterDirectReference(prop)">{{prop.name}}</input> <input ng-if="!prop.array && !prop.type && isEditable(prop.name) && prop.name != \'id\'" type="text" ng-model="model[prop.name]" ng-change="onChange(model, prop.name)"> <input ng-if="!prop.array && !prop.type && !isEditable(prop.name) || prop.name == \'id\'" type="text" ng-model="model[prop.name]" disabled="true"> <input ng-if="isEditable(prop.name) && (prop.array || prop.hashtable)" flex   ng-model="model[prop.name]" ng-click="selectModel(prop.type, prop.name)"></input> <input ng-if="!isEditable(prop.name) && (prop.array || prop.hashtable)" flex  ng-model="model[prop.name]" >{{model[prop.name]}}</input> </div> </div>',
        scope: {
          model: '=model',
          edit: '=?edit',
          onselect: '&',
