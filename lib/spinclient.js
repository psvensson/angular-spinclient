  ...
  angular.module('ngSpinclient', ['uuid4', 'ngMaterial']).factory('spinclient', function(uuid4, $q, $rootScope) {
    ...
    'spinclient', '$mdDialog', function(client, $mdDialog) {
      return {
        restrict: 'AE',
        replace: true,
        template: '<div> <md-subheader class="md-no-sticky" style="background-color:#ddd"> <md-icon md-svg-src="assets/images/ic_folder_shared_24px.svg" ></md-icon> {{model.type}} {{objects[model.id].name}} </md-subheader> <md-list flex> <md-list-item ng-repeat="prop in listprops" flex layout="row"  layout-fill> <md-input-container layout-padding layout-fill style="min-height:20px"> <label flex="25"> {{prop.name}} </label> <span flex ng-if="prop.type && prop.value && !prop.hashtable && !prop.array"> <md-button ng-click="enterDirectReference(prop)">{{prop.name}}</md-button> > </span> <input flex="50" ng-if="!prop.array && !prop.type && isEditable(prop.name) && prop.name != \'id\'" type="text" ng-model="model[prop.name]" ng-change="onChange(model, prop.name)"> <input flex="50" ng-if="!prop.array && !prop.type && !isEditable(prop.name) || prop.name == \'id\'" type="text" ng-model="model[prop.name]" disabled="true"> <div layout-align="right" ng-if="accessrights[prop.type].create && (prop.array || prop.hashtable)"><md-button class="md-raised" ng-click="addModel(prop.type, prop.name)">New {{prop.type}}</md-button></div> <div layout-align="right" ng-if="accessrights[model.type].write && (prop.array || prop.hashtable)"><md-button class="md-raised" ng-click="selectModel(prop.type, prop.name)">Add {{prop.type}}</md-button></div> <spinlist ng-if="isEditable(prop.name) && prop.array" flex search="local" listmodel="prop.type" edit="edit" list="model[prop.name]" onselect="onselect" ondelete="ondelete"></spinlist> <spinlist ng-if="!isEditable(prop.name) && prop.array" flex  listmodel="prop.type" list="model[prop.name]" onselect="onselect"></spinlist> <spinhash ng-if="prop.hashtable" flex  listmodel="prop.type" list="prop.value" onselect="onselect"></spinhash> </md-input-container> </md-list-item> </md-list> </div>',
        scope: {
          model: '=model',
          edit: '=?edit',
          onselect: '&',
    ...
    'spinclient', function(client) {
      return {
        restrict: 'AE',
        replace: false,
        template: '<div> <span ng-repeat="crumb in breadcrumbs"> <md-button ng-click="crumbClicked(crumb)">{{crumbPresentation(crumb)}}</md-button> > </span> <md-divider></md-divider> <spinmodel model="selectedmodel" edit="edit" onselect="onselect" ondelete="ondelete" hideproperties="hideproperties" style="height:400px;overflow:auto"></spinmodel> </div>',
        scope: {
          model: '=model',
          edit: '=edit',
          ondelete: '=ondelete',
    ...
    'spinclient', function(client) {
      return {
        ...
        controller: function($scope) {
            ...
            if ($scope.list) {
              ...
              for (j = 0, len = ref.length; j < len; j++) {
                ...
                if (client.objects[modelid]) {
                  results1.push($scope.addExpandedModel(client.objects[modelid]));
                } else {
                  results1.push(client.emitMessage({
                    ...
                    return $scope.addExpandedModel(o);
                  }, failure));
                }
              }
              return results1;
          ...
          $scope.addExpandedModel = function(o) {
            var i, j, len, modid, ref, results1;
            ref = $scope.list;
            results1 = [];
            for (i = j = 0, len = ref.length; j < len; i = ++j) {
              modid = ref[i];
    ...
    'spinclient', '$mdDialog', function(client, $mdDialog) {
      return {
        restrict: 'AE',
        replace: true,
        template: '<div layout="row"layout-fill style="margin-right:16px" style="background-color: #cdefaa"> <div  flex="{{calcflex}}" ng-repeat="prop in listprops" > <span flex ng-if="prop.type && prop.value && !prop.hashtable && !prop.array" ng-click="enterDirectReference(prop)">{{prop.name}}</span> <input flex ng-if="!prop.array && !prop.type && isEditable(prop.name) && prop.name != \'id\'" type="text" ng-model="model[prop.name]" ng-change="onChange(model, prop.name)"> <input flex ng-if="!prop.array && !prop.type && !isEditable(prop.name) || prop.name == \'id\'" type="text" ng-model="model[prop.name]" disabled="true"> <span flex ng-if="isEditable(prop.name) && (prop.array || prop.hashtable)" ng-model="model[prop.name]" ng-click="selectModel(prop.type, prop.name)">{{prop.name}}</span> <span flex ng-if="!isEditable(prop.name) && (prop.array || prop.hashtable)" >{{prop.name}}</span> </div> </div>',
        scope: {
          model: '=model',
          edit: '=?edit',
          onselect: '&',
        ...
        controller: function($scope) {
          ...
          $scope.renderModel = (function(_this) {
            return function() {
              $scope.listprops = [];
              return client.getModelFor($scope.model.type).then(function(md) {
                var foo, i, j, len, modeldef, notshow, prop, ref;
                modeldef = {};
                md.forEach(function(modelprop) {
                  return modeldef[modelprop.name] = modelprop;
                });
                if ($scope.model) {
                  ...
                  return $scope.calcflex = parseInt((1 / md.length) * 100);
                }
              });
            };
          })(this);
    ...
    'spinclient', '$mdDialog', function(client, $mdDialog) {
      return {
        restrict: 'AE',
        replace: false,
        template: '<div> <md-subheader class="md-no-sticky" style="background-color:#ddd"> <md-icon md-svg-src="assets/images/ic_apps_24px.svg" ></md-icon> Grid of {{listmodel}}s</md-subheader> <md-grid-list md-cols="{{ocols}}" md-row-height="35px" style="margin-right:16px"> <md-grid-tile ng-repeat="prop in objectmodel" style="background-color: #cacaca"> {{prop.name}} </md-grid-tile> <md-grid-tile ng-if="!onselect" ng-repeat="cell in cells" style="height:15px" layout-fill> <md-button raised ng-if="onselect" ng-click="selectItem(cell.item)">Select</md-button> <span flex ng-if="cell.prop.type && cell.prop.value && !cell.prop.hashtable && !cell.prop.array" ng-click="enterDirectReference(prop)">{{cell.item[cell.prop.name]}}</span> <input layout-fill flex ng-if="!cell.prop.array && !cell.prop.type &&  isEditable(cell.prop.name) && cell.prop.name != \'id\'" type="text" ng-model="cell.item[cell.prop.name]" ng-change="onChange(cell.item, cell.prop.name)"> <input layout-fill flex ng-if="!cell.prop.array && !cell.prop.type && !isEditable(cell.prop.name) && cell.prop.name != \'id\'" type="text" ng-model="cell.item[cell.prop.name]" disabled="true"> <span flex ng-if="isEditable(cell.prop.name) && (cell.prop.array || cell.prop.hashtable)" ng-model="cell.item[cell.prop.name]" ng-click="selectModel(cell.item, cell.prop.type, cell.prop.name)">{{cell.item[cell.prop.name].length}} {{cell.prop.name}}</span> <span flex ng-if="!isEditable(cell.prop.name) && (cell.prop.array || cell.prop.hashtable)" >{{cell.item[cell.prop.name].length}} {{cell.prop.name}}</span> </md-grid-tile> </md-grid-list> </div>',
        scope: {
          list: '=list',
          listmodel: '=listmodel',
          onselect: '&',
        ...
        controller: function($scope) {
          ...
          return $scope.selectModel = function(item, type, propname) {
            ...
            return $mdDialog.show({
              ...
              template: '<md-dialog aria-label="selectdialog"> <md-content> <md-button class="md-raised" ng-click="addModel(item, type, propname)">New {{type}}</md-button> <spinlist listmodel="type" list="list" edit="true" onselect="onselect" ondelete="ondelete"></spinlist> </md-content> </md-dialog>'
            });
          };
        }
      };
