  ...
  angular.module('ngSpinclient', ['uuid4', 'ngMaterial']).factory('spinclient', function(uuid4, $q, $rootScope) {
    ...
    'spinclient', function(client) {
      return {
        restrict: 'AE',
        replace: false,
        template: '<div> <md-subheader class="md-no-sticky" style="background-color:#ddd"> <md-icon md-svg-src="assets/images/ic_apps_24px.svg" ></md-icon> Grid of {{listmodel}}s</md-subheader> <md-grid-list md-cols="{{ocols}}" md-row-height="35px" style="margin-right:16px"> <md-grid-tile ng-repeat="prop in objectmodel" style="background-color: #cacaca"> {{prop.name}} </md-grid-tile> <md-grid-tile ng-repeat="cell in cells" style="height:15px" layout-fill ng-click="selectItem(cell.item)"> <span flex ng-if="cell.prop.type && cell.prop.value && !cell.prop.hashtable && !cell.prop.array" ng-click="enterDirectReference(prop)">{{cell.item[cell.prop.name]}}</span> <input lyout-fill flex ng-if="!cell.prop.array && !cell.prop.type && isEditable(cell.prop.type) && cell.prop.name != \'id\'" type="text" ng-model="cell.item[cell.prop.name]" ng-change="onChange(cell.item, cell.prop.name)"> <input layout-fill flex ng-if="!cell.prop.array && !cell.prop.type && !isEditable(cell.prop.type) || cell.prop.name == \'id\'" type="text" ng-model="cell.item[cell.prop.name]" disabled="true"> <span flex ng-if="isEditable(cell.prop.name) && (cell.prop.array || cell.prop.hashtable)" ng-model="cell.item[cell.prop]" ng-click="selectModel(cell.item.type, cell.prop.name)">{{cell.item[cell.prop.name]}}</span> <span flex ng-if="!isEditable(cell.prop.name) && (cell.prop.array || cell.prop.hashtable)" >{{cell.item[cell.prop.name].length}} {{cell.prop.name}}</span> </md-grid-tile> </md-grid-list> </div>',
        scope: {
          list: '=list',
          listmodel: '=listmodel',
          onselect: '&',
                  ...
                  if (modid === o.id) {
                    ...
                    results2.push((function() {
                      ...
                      for (k in ref2) {
                        ...
                        results3.push(console.log('adding cell ' + o.name));
                      }
                      return results3;
                    })());
                  } else {
