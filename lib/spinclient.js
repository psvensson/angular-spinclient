  ...
  angular.module('ngSpinclient', ['uuid4', 'ngMaterial']).factory('spinclient', function(uuid4, $q, $rootScope) {
                  ...
                  for (i = j = 0, len = md.length; j < len; i = ++j) {
                    ...
                    if (prop.name !== 'id' && !notshow && prop.name !== $scope.activeField && $scope.model[prop.name]) {
                      if (prop.name.indexOf('At') > -1) {
                        val = new Date($scope.model[prop.name]);
                      } else {
    ...
    'spinclient', function(client) {
      return {
        restrict: 'AE',
        replace: true,
        template: '<div > <spinlist listmodel="listmodel" list="list" onselect="onourselect" replace="replace" ondelete="ondelete" edit="edit" search="search" searchfunc="searchfunc"></spinlist> </div>',
        scope: {
          listmodel: '=listmodel',
          edit: '=edit',
          onselect: '&',
