  ...
  angular.module('ngSpinclient', ['uuid4', 'ngMaterial']).factory('spinclient', function(uuid4, $q, $rootScope) {
    ...
    'spinclient', '$mdDialog', function(client, $mdDialog) {
      return {
        restrict: 'AE',
        replace: true,
        template: '<div style="padding:15px"> <md-subheader class="md-no-sticky" style="background-color:#ddd"> <md-icon md-svg-src="assets/images/ic_folder_shared_24px.svg" ></md-icon> {{model.type}} {{objects[model.id].name}} </md-subheader> <md-list flex> <md-list-item ng-repeat="prop in listprops" flex layout="row"  layout-fill> <md-input-container layout-padding layout-fill style="min-height:20px"> <label flex="25"> {{prop.name}} </label> <span flex ng-if="prop.type && prop.value && !prop.hashtable && !prop.array"> <md-button ng-click="enterDirectReference(prop)">{{prop.name}}</md-button> > </span> <input flex="50" ng-if="!isdate(prop.name) && !prop.array && !prop.type && isEditable(prop.name) && prop.name != \'id\'" type="text" ng-model="prop.value" ng-change="onChange(model, prop.name, prop.value)"> <input flex="50" ng-if="!isdate(prop.name) && !prop.array && !prop.type && !isEditable(prop.name) || prop.name == \'id\'" type="text" ng-model="prop.value" disabled="true"> <input flex="50" ng-if="isdate(prop.name)" type="datetime" value="{{prop.value}}" ng-disabled="true"> <spinlist ng-if="isEditable(prop.name) && prop.array" flex search="local" listmodel="prop.type" edit="edit" list="model[prop.name]" onselect="onselect" ondelete="ondelete"></spinlist> <spinlist ng-if="!isEditable(prop.name) && prop.array" flex  listmodel="prop.type" list="model[prop.name]" onselect="onselect"></spinlist> <spinhash ng-if="prop.hashtable" flex  listmodel="prop.type" list="prop.value" onselect="onselect"></spinhash> <div layout-align="right" ng-if="accessrights[prop.type].create && (prop.array || prop.hashtable)"><md-button class="md-raised" ng-click="addModel(prop.type, prop.name)">New {{prop.type}}</md-button></div> <div layout-align="right" ng-if="accessrights[model.type].write && (prop.array || prop.hashtable)"><md-button class="md-raised" ng-click="selectModel(prop.type, prop.name)">Add {{prop.type}}</md-button></div> </md-input-container> </md-list-item> </md-list> </div>',
        scope: {
          model: '=model',
          edit: '=?edit',
          onselect: '&',
            ...
            for (i = l = 0, len = slice.length; l < len; i = ++l) {
              ...
              if (client.objects[modelid]) {
                ...
                if (modelid) {
                  ...
                  results1.push(console.dir(slice));
                }
              }
            }
            return results1;
